...
class PNFW_Notifications_Android extends PNFW_Notifications {
     ...
     if (is_null($result->getMessageId())) {
      switch ($result->getErrorCode()) {
       ...
       case 'InternalServerError':
        $this->log_gcm_error($result->getErrorCode(), __('You could retry to send it late in another request.', 'push-notifications-for-wp'));
        break;

       case 'MissingRegistration':
        $this->log_gcm_error($result->getErrorCode(), __('Check that the request contains a registration ID.', 'push-notifications-for-wp'));
        break;

       case 'InvalidPackageName':
        $this->log_gcm_error($result->getErrorCode(), __('Make sure the message was addressed to a registration ID whose package name matches the value passed in the request.', 'push-notifications-for-wp'));
        break;

       case 'MismatchSenderId':
        $this->log_gcm_error($result->getErrorCode(), __('Make sure the Sender Id (Project Number) in your App is correct.', 'push-notifications-for-wp'));
        break;

       case 'MessageTooBig':
        $this->log_gcm_error($result->getErrorCode(), __('Check that the total size of the payload data included in a message does not exceed 4096 bytes.', 'push-notifications-for-wp'));
        break;

       case 'InvalidDataKey':
        $this->log_gcm_error($result->getErrorCode(), __('Check that the payload data does not contain a key that is used internally by GCM.', 'push-notifications-for-wp'));
        break;

       case 'InvalidTtl':
        $this->log_gcm_error($result->getErrorCode(), __('Check that the value used in time_to_live is an integer representing a duration in seconds between 0 and 2,419,200.', 'push-notifications-for-wp'));
        break;

       case 'DeviceMessageRateExceed':
        $this->log_gcm_error($result->getErrorCode(), __('Reduce the number of messages sent to this device.', 'push-notifications-for-wp'));
        break;
      }
     }
     else {
 ...
 private function log_gcm_error($error_code, $error_message) {
